  ...
  return (
    <>
      ...
      >
        <motion.button
          id="menu-button"
          className={cn("flex flex-col justify-center gap-y-1 max-w-6 max-h-6")}
        ...
        <ul
          className="absolute left-0 top-10 overflow-hidden bg-white/90 dark:bg-background/90 pl-8 pt-1 backdrop-blur-lg"
          data-menu-ul
        >
          {NAV_LINKS.map((link, index) => (
            <motion.li key={index}>
              <Link
                ...
                className={cn(
                  "hover:text-accent relative w-fit text-sm font-medium text-gray-900 dark:text-foreground transition-colors duration-300"
                )}
              >
                {t(`${link.route}`)}
                <span
